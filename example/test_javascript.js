...
const fs = require('fs');
const { ProtocolLogin, Account } = require('./export/javascript/protocol.js');

function verifyLogin(original, decoded) {
...
function main() {
    console.log("=== JavaScript Protocol Test ===\n");
    ...
    fs.writeFileSync("test_output_javascript.json", jsonData);
    ...
    fs.writeFileSync("test_output_javascript.bin", Buffer.from(binaryData));
    ...
    console.log("6. Testing truncated binary throws...");
    try {
        const bad = new ProtocolLogin();
        bad.decode(binaryData.slice(0, 8));
        console.log("   ✗ Truncated binary should throw");
        process.exit(1);
    } catch (e) {
        if (String(e.message || e).includes("unexpected end of data")) {
            console.log("   ✓ Truncated binary throws");
        } else {
